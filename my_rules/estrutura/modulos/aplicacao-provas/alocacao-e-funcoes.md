...
`useCoordenadoresProva.tsx` só considera "elegível" para virar coordenador um `colaboradores_prova` cuja `funcao_id` esteja em `FUNCOES_COORDENACAO` (acima). A concessão em si é a RPC **`conceder_coordenador(p_colaborador_prova_id)`** (migration `20260912165246`), que insere nas duas tabelas em transação — detalhada em [`auth-e-permissoes.md`](../../transversais/auth-e-permissoes.md), não duplicada aqui.

> 🔴 **A elegibilidade tem DOIS requisitos, e o segundo é novo (2026-09-12): o colaborador precisa JÁ TER CONTA.** Antes o diálogo criava a conta ali mesmo, com e-mail e senha digitados pelo admin; hoje o acesso usa `colaboradores.user_id`, e quem não o tem aparece na lista **desabilitado, com o motivo**. ⚠️ Os UUIDs de `FUNCOES_COORDENACAO` agora estão em **dois** lugares — o hook e a RPC. O caso 0.1 da bateria afirma que as duas linhas existem, para a quebra dar sinal em vez de esvaziar a lista em silêncio.

> ✅ **E a regra virou invariante de banco.** O trigger `check_coordenador_prova_coerente_trigger` exige que `coordenadores_prova.user_id` seja o `user_id` do colaborador daquela alocação, e que `prova_id` seja a prova dela. 🔴 **A checagem do `user_id` é CONDICIONAL de propósito** — só vale quando o cadastro tem vínculo: o dump **não** traz `colaboradores.user_id` (o backfill está no `seed.pos.sql`, que roda *depois*), então exigi-lo sempre derrubaria todo `db reset` e o bootstrap de produção, que lê o mesmo arquivo. Medido em 2026-09-12: as 10 linhas existentes já eram coerentes nos dois critérios, então não houve saneamento.
