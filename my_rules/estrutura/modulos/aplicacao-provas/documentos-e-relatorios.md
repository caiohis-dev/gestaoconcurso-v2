...
🔵 **"Já atuaram" e a capacidade total vêm da RPC `totais_do_dashboard()` desde 2026-09-24** (migration `20260925005346`, `SECURITY INVOKER`, bateria `docs/bateria-totais-do-dashboard.sql`). ⚠️ Este parágrafo dizia que o Dashboard usava só `count/head` "para evitar trazer os dados" — **dois cards não usavam**: liam `colaboradores_prova` e `sala_prova` **inteiras** para contar distintos num `Set` e somar no cliente. O PostgREST corta em `max_rows` (1000) **sem erro**, e `colaboradores_prova` estava com **977** linhas (555 em 12/09) — o card passaria a mentir na próxima prova. 🔴 **Não troque de volta por `select` + conta no cliente.** Coberto por `src/pages/Dashboard.ui.test.tsx`, que afirma também a AUSÊNCIA: a tela não lê mais `colaboradores_prova`.
