...
🧪 **O `ColaboradorDialog` tem bateria de interação desde 2026-07-26** (24 testes), cobrindo os três comportamentos que só existem na tela: a **âncora de identidade** (linha vinculada tem `colab_email` somente-leitura e ele **sai do payload** do update), o **modo público** (fala direto com a EF, não se deixa fechar por Esc, e traduz "CPF já cadastrado"/"e-mail em uso") e a **normalização** do que é enviado. Dois defeitos saíram dali, ambos no [`backlog.md`](../../../backlog.md): o `padStart` do CPF rodando antes da validação, e o aviso final sendo `aria-hidden`.
