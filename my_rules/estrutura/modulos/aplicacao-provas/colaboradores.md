...
🔵 **As recusas de CHECK chegam traduzidas desde 2026-09-24.** `mensagemRecusaCheck` (`useColaboradores.tsx`) tem uma frase por constraint — as **12** de `colaboradores`, medidas em `pg_constraint` no dia — e é usada no cadastro e na edição do `ColaboradorDialog`, antes da tradução de duplicidade. Casa o nome **só em `message`**: o `details` de uma violação de CHECK traz a linha inteira, com PII. ⚠️ CHECK nova na tabela sem ramo ali cai numa frase genérica, que não nomeia o campo — acrescente o ramo no mesmo passe da migration. 🔵 O `PerfilColaborador` usa o mesmo helper desde o mesmo dia, por `mensagemRecusaPerfil`. As RPCs dele (`update_meu_colaborador`, `update_meus_dados_bancarios`) recusam de **dois** jeitos, medidos chamando-as como colaborador: **`P0001`** com frase escrita pela própria RPC — inclusive a duplicidade, que a `update_meu_colaborador` **captura** e relança como *"Este e-mail ou chave PIX já está em uso…"* — e **`23514`** de CHECK, que elas não capturam. ⚠️ Até então a tela **descartava** o `P0001` em favor de "Não foi possível salvar" e tinha um ramo `23505` por índice que **nunca rodava** (a RPC engole o `unique_violation`; e a matrícula nem é mais única — os índices são CPF, PIS, e-mail e PIX). Os dois defeitos que sobraram na RPC estão no backlog.

Consequência no app: salvar cadastro agora pode falhar com `23505`. `PerfilColaborador` mostra a frase que a própria RPC escreve — ⚠️ esta linha dizia que ele *"traduz o erro (mensagem específica para e-mail e para chave PIX)"*, e isso **nunca rodou**: o `23505` não chega, a RPC o relança como `P0001` (medido em 2026-09-24) — e o `CadastroLote` classifica as linhas recusadas como "E-mail duplicado" / "Chave PIX duplicada".
...
🧪 **O `ColaboradorDialog` tem bateria de interação desde 2026-07-26** (24 testes), cobrindo os três comportamentos que só existem na tela: a **âncora de identidade** (linha vinculada tem `colab_email` somente-leitura e ele **sai do payload** do update), o **modo público** (fala direto com a EF, não se deixa fechar por Esc, e traduz "CPF já cadastrado"/"e-mail em uso") e a **normalização** do que é enviado. 🔵 **Desde 2026-09-24 o clique fora NÃO fecha o diálogo em modo nenhum** (`onInteractOutside` com `preventDefault`; antes só o `publicMode` barrava) — fechar descartava o preenchimento calado, inclusive no *Editar Colaborador* de `/colaboradores`. O Esc segue fechando no modo administrativo. ⚠️ O teste dispara o `pointerdown` **depois de um tick**: o Radix registra o listener num `setTimeout(0)`, e disparar antes passava verde sem a proteção. Os dois defeitos que saíram dali — o `padStart` do CPF rodando antes da validação e o aviso `aria-hidden` — **foram corrigidos no mesmo dia**, e estão explicados nas duas seções acima.
