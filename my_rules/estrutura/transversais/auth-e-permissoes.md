...
**Rate limit compartilhado.** As duas portas gastam do **mesmo** orçamento — o escopo `acesso`, 5/10 min por IP (🔵 era 5/15 até 2026-09-24), na tabela `reivindicacao_rate_limit`. Separadas, o atacante somaria 5 pelo CPF **mais** 5 pelo e-mail.
...
|---|---|---|---|
| `acesso` | `reivindicar-acesso` + `recuperar-senha` | **5 / 10 min** | orçamento compartilhado, como acima. 🔵 **Era 5/15 até 2026-09-24**, folgado por decisão do usuário: o teto conta **toda** requisição — a bem-sucedida também, e antes de saber o resultado —, então uma coordenação disparando o acesso de vários colaboradores do mesmo IP esbarrava nele. Um botão de "reenviar convite" foi preterido por dar mais trabalho ao coordenador. O 429 deste escopo tem **frase própria**: *"Sistema com excesso de acessos. Tente novamente após 10 minutos."* (`mensagemDoBloqueio`); ⚠️ o `ReivindicarAcessoCard` **não lê** a frase da EF, escreve a dele — mudou uma, mude a outra |
...
- **Reivindicação (os 759 que já eram cadastrados, sem conta):** em `/auth`, **"Estou sem minha senha"** (a porta única — era "Primeiro acesso" até 2026-07-20) abre o `ReivindicarAcessoCard` → CPF → a Edge Function **`reivindicar-acesso`** localiza o cadastro e devolve **`{existe, ja_vinculado, email_mascarado}`** (o e-mail inteiro nunca sai do servidor), disparando um `generateLink` enviado com HTML da FEVRE via `send-email` (🔵 era `'invite'` fixo até 2026-09-19; hoje o helper escolhe entre `invite` e `recovery`). A pessoa clica, cai em `/redefinir-senha`, define a senha, entra. Rate limit de 5/10 min por IP (🔵 era 5/15 até 2026-09-24; tabela `reivindicacao_rate_limit`, migration `20260714201650`), **compartilhado com a `recuperar-senha`**.
- **Cadastro público (subetapa 2C):** `/cadastro-publico` → CPF novo → o `ColaboradorDialog` em `publicMode` (sem código de 4 dígitos, e-mail obrigatório) → `public-create-colaborador` cria a linha e **dispara o mesmo link** da reivindicação (🔵 `invite` ou `recovery`, conforme o e-mail já ter conta — até 2026-09-19 era invite fixo, e a linha nascia sem que nada chegasse quando a conta existia). Se o CPF já existe, a página mostra o `ReivindicarAcessoCard` inline — as duas portas convergem.
- **O vínculo é automático, nos triggers — dois, desde 2026-09-19.** A regra mora numa função só, **`vincular_colaborador_a_conta(p_user_id, p_email)`** (migration `20260919121555`): se o e-mail da conta casa com um colaborador de **`user_id IS NULL`**, ela preenche `user_id` e concede **`colaborador`**. Quem a chama são **`on_auth_user_created` → `handle_new_user`** (nascimento da conta, o de sempre) e **`on_auth_user_signin` → `vincular_colaborador_no_signin`** (`AFTER UPDATE OF last_sign_in_at, email_confirmed_at`). 🔵 Esta linha dizia só "no trigger", e o vínculo valia **apenas para conta nova** — era esse o buraco: quem já tinha conta não se vinculava por caminho nenhum. Isso vale para *qualquer* conta nova — reivindicação, cadastro público ou uma conta criada por admin. Nada vem do cliente; o casamento é por `auth.users.email` (único) contra o índice único de `colab_email`. É o backfill dos 12 virado mecanismo contínuo.
- 🔵 **Desde 2026-09-20 o gatilho de login faz MAIS UMA coisa: carimba `colaboradores.colab_ultimo_acesso` com `NEW.last_sign_in_at`** (migration `20260921002249_carimbar_ultimo_acesso_no_login.sql`). A coluna estava sem escritor desde 15/07 e a tela `/colaboradores` mentia para **263 de 263** pessoas em produção. 🔴 **O carimbo mora num bloco `EXCEPTION` SEPARADO do vínculo, e isso é estrutural:** fundidos, a exceção do carimbo desfaz o vínculo junto — a subtransação do bloco é anulada inteira. Falsificado no caso 14 de `docs/bateria-vinculo-colaborador.sql`. ⚠️ O carimbo só acontece quando `last_sign_in_at` **muda** — confirmar e-mail não é acesso, e sem essa guarda a coluna voltaria a mentir por outro caminho (caso 10). ⚠️ **Não houve backfill** (decisão do usuário): cada pessoa se corrige no primeiro login seguinte.
- **O link é compartilhado:** o helper `supabase/functions/_shared/enviar-link-acesso.ts` (generateLink + HTML da FEVRE + `send-email`) é usado por `reivindicar-acesso` **e** `public-create-colaborador`. 🔵 Desde 2026-09-19 o padrão dele é **`'auto'`**: consulta o Auth (`_shared/auth-lookup.ts`) e escolhe entre `invite` e `recovery`. Esta linha dizia "o **invite** é compartilhado" — era o padrão fixo, e o defeito.

  🔵 **Desde 2026-09-20 o helper também grava a trilha de envio** — tabela `log_envio_link_acesso` (migration `20260921005259_trilha_envio_link_acesso.sql`), escrita NUM PONTO ÚNICO dentro do helper (nunca em cada chamador — a mesma lição do `registrarFalhaDeEnvio`). Responde "o e-mail saiu?" sem depender dos carimbos do GoTrue. RLS: SELECT só admin. Best-effort — falha da gravação nunca derruba o envio real. Ver [`../../analises/analise-ultimo-acesso-e-convite.md`](../../analises/analise-ultimo-acesso-e-convite.md) §6.2.
...
Travar o campo (Etapa 1) impede o estrago novo, mas não conserta quem já está preso. A **Edge Function `corrigir-email-acesso`** é a saída deliberada — no lugar certo, e não como edição casual de formulário. Ponto de entrada: um link discreto sob o campo travado do `ColaboradorDialog` ("Clique aqui caso queira corrigir o e-mail de acesso" (🔵 era "O e-mail está errado e ele nunca conseguiu entrar?" até 2026-09-24)), que abre o `CorrigirEmailAcessoDialog`.
...
- **O helper compartilhado mudou:** `_shared/enviar-link-acesso.ts` ganhou `tipo?: 'invite' | 'recovery'`. ⚠️ **O padrão era `'invite'`, e `reivindicar-acesso` e `public-create-colaborador` seguiam intactos — as duas coisas deixaram de valer em 2026-09-19.** O padrão virou **`'auto'`** e os dois passaram a mandar `recovery` quando o e-mail já tem conta. Quem passa o tipo **explícito** (esta função, com `'recovery'`, e a `recuperar-senha`, com os dois) é que continua intacto.
...
> 1. Ela criava a conta **antes** de gravar `colab_email`, então `handle_new_user` não casava nada: `colaboradores.user_id` ficava NULL e o papel `colaborador` não era concedido — **coordenador que não é colaborador**. E sem conserto no app: reivindicar pelo CPF disparava `generateLink('invite')` num e-mail que já tem conta, o invite falhava, e `reivindicar-acesso` respondia sucesso assim mesmo. 🟢 **Essa segunda parte foi CONSERTADA em 2026-09-19** — é o item de backlog que este aviso anunciava: o helper agora manda `recovery` quando a conta existe, o `reivindicar-acesso` loga a falha em vez de engoli-la, e o gatilho de login faz o vínculo que faltava.
