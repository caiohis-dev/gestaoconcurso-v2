...
| 🔴 **Prova ou unidade FINALIZADA congela as salas distribuídas** | trigger `check_sala_de_prova_finalizada` (03/08), SQLSTATE `PF001`, em INSERT/UPDATE/DELETE. Congela nos **dois** níveis (`provas.prova_finalizada` e `prova_unidades.unidade_finalizada`) — e o segundo o **coordenador** aciona. ⚠️ Sem exceção por role: quem sustenta o `db reset` é o `SET session_replication_role = replica` do próprio dump. ⚠️ Desvincular unidade de prova finalizada passa a ser recusado (a RPC apaga estas salas). Bateria: [`../../../docs/bateria-salas-renumeracao.sql`](../../../docs/bateria-salas-renumeracao.sql) §4 |
| Número de sala único por (prova, unidade) — **inclusive ao renumerar** | índice `salas_prova_distribuidas_prova_unidade_numero_key` (26/07) + RPC `salvar_salas_distribuidas` (03/08), que grava o lote em transação e renumera em **dois passos**. ⚠️ **Deixar a chave `DEFERRABLE` quebra o `db reset`** — o dump usa `ON CONFLICT DO NOTHING` sem alvo e o Postgres recusa árbitro deferrable (`55000`). Bateria: [`../../../docs/bateria-salas-renumeracao.sql`](../../../docs/bateria-salas-renumeracao.sql) · ver [`../modulos/aplicacao-provas/provas-e-unidades.md`](../modulos/aplicacao-provas/provas-e-unidades.md) |
| Hierarquia de papéis (`superadmin ⇒ admin`) | dentro do `has_role` |
| Inscrição de candidato única por (**edital**, **nº de inscrição**) | `candidatos_edital_inscricao_key` (01/08, migration `20260801193530`) — ver [`../modulos/candidatos/00-modulo.md`](../modulos/candidatos/00-modulo.md). ⚠️ Era `(edital, CPF, cargo_id, nº)` com `NULLS NOT DISTINCT` até 31/07; CPF e cargo **saíram da identidade**. Bateria: [`../../../docs/bateria-chave-natural-candidatos.sql`](../../../docs/bateria-chave-natural-candidatos.sql) |
| 🔴 **O edital de uma prova é IMUTÁVEL depois de definido** | trigger `check_prova_edital_imutavel` (02/08), SQLSTATE `PE001`. ⚠️ A condição é *"`edital_id` **já tinha valor** e mudou"*, **não** *"mudou"*: o backfill do `seed.pos.sql` faz `NULL → valor` **depois** das migrations, e travá-lo quebraria todo `db reset`. `valor → NULL` também é recusado. Bateria: [`../../../docs/bateria-prova-edital-imutavel.sql`](../../../docs/bateria-prova-edital-imutavel.sql) · ver [`../modulos/aplicacao-provas/provas-e-unidades.md`](../modulos/aplicacao-provas/provas-e-unidades.md) |
| 🔴 **Cargo com INSCRITO em algum edital é IMUTÁVEL** (não altera nem exclui) | trigger `check_cargo_nao_alteravel_com_inscritos` (01/08), SQLSTATE `CG001` + `candidatos_cargo_id_fkey` RESTRICT. ⚠️ **Entre 31/07 e 01/08 a regra era mais larga** (migration `20260731100000`: qualquer menção trancava) e travava o cargo já no **passo 3 do assistente**, que grava o apelido antes de qualquer inscrito existir — apelido **não** tranca mais. A janela para limpar um nome vai até a primeira importação que o use. Ver [`../modulos/candidatos/cargos.md`](../modulos/candidatos/cargos.md) |
...
| 🔵 **Quem é retirado da alocação automática não entra no plano** (05/08): a marcação é por `(prova, candidato)` — o Edital 001 tem DUAS provas, e uma coluna em `candidatos` afetaria as duas em silêncio. Prova finalizada congela a marcação (`PF001`); marcar quem já está em sala À MÃO é recusado (`AL011`), e colocar à mão **apaga** a marcação — as duas dizem "o plano não mexe nesta pessoa", e manter as duas seria estado redundante | migrations `20260805205719` e `20260806011211` — tabela `candidatos_fora_do_automatico`, triggers `check_candidato_fora_do_automatico` e `alocacao_manual_limpa_marcacao`. ⚠️ FK de candidato em **CASCADE** (exceção ao padrão RESTRICT): a marcação é anotação sem valor próprio, mas o preço é perda silenciosa na reimportação — mitigada por AVISO, não por barreira. Bateria: [`../../../docs/bateria-alocacao-candidatos.sql`](../../../docs/bateria-alocacao-candidatos.sql) casos 16a–16i |
| 🔵 **A alocação de candidato em sala é coerente e finita** (04/08): um candidato tem UMA sala por prova; a sala pertence à prova; o candidato é do edital da prova; sala não passa da capacidade; capacidade não desce abaixo da ocupação; prova/unidade finalizada congela | migration `20260804225156` — `candidatos_alocacao_prova_candidato_key`, FK composta `candidatos_alocacao_sala_prova_fkey`, trigger `check_candidato_alocacao` (PF001/AL005/AL006), trigger `check_sala_reducao_capacidade` (AL007). ⚠️ **A RPC de distribuição mudou em 05/08**: `distribuir_candidatos_da_prova` foi DROPADA e substituída por `aplicar_plano_de_alocacao(prova, plano jsonb)` — o admin passou a escolher a unidade de cada bloco, arrastando (`AL008`/`AL009`/`AL010` novos). Bateria: [`../../../docs/bateria-alocacao-candidatos.sql`](../../../docs/bateria-alocacao-candidatos.sql) · ver [`../modulos/alocacao-candidatos/00-modulo.md`](../modulos/alocacao-candidatos/00-modulo.md) |
| 🔵 **Candidato ALOCADO não se apaga** (04/08): a troca total, excluir um e "limpar edital" são recusados enquanto houver alocação — decisão do usuário: reimportar vira ação em dois passos | FK `candidatos_alocacao_candidato_id_fkey` **RESTRICT**. ⚠️ É o RESTRICT indireto clássico: os tradutores das telas pai (`mensagemErroImportacao` e `mensagemErroDesvinculoUnidade`) ganharam o ramo no mesmo passe |
| Cargo com **inscrito** não se exclui; cargo só com apelido, sim | `candidatos.cargo_id` **RESTRICT** e `cargo_apelidos.cargo_id` **CASCADE** (conferido no banco em 01/08) — opostas de propósito: *"apelido é atalho de digitação, candidato é gente"*. ⚠️ **As duas foram RESTRICT entre 31/07 e 01/08.** O CASCADE é **perda silenciosa** — some a memória de "texto sujo → cargo" —, e quem a torna visível é o aviso no diálogo de `Cargos.tsx`, com teste próprio. Ver [`../modulos/candidatos/cargos.md`](../modulos/candidatos/cargos.md) |
...
- **A numeração sequencial de salas** continua sendo calculada no cliente. Movê-la para o banco (uma sequence por unidade+andar) foi considerado desproporcional; o UNIQUE já converte o pior caso em erro visível.
  ...
  > 🔵 **Meio-termo em 2026-08-03:** *qual* número a próxima sala recebe segue sendo conta do cliente (`numerosDoLote`, em `lib/salas.ts`), mas a **coerência** do número virou barreira de banco — `chk_sala_numero_casa_com_andar` exige `sala_numero = sala_andar × 100 + (1..99)` em `sala_prova`. Foi o suficiente para fechar o caso que a UNIQUE não pegava: a edição trocando **só** o andar, ou **só** o número, deixando os dois divergentes sem erro nenhum. ⚠️ A CHECK **não** vai para `salas_prova_distribuidas`: a RPC de renumeração escreve um valor transitório fora da faixa, e nenhum transitório satisfaria a regra.
