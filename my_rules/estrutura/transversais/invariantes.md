...
| 🔴 **Cargo com menção em outra tabela é IMUTÁVEL** (não altera nem exclui) | trigger `check_cargo_nao_alteravel_em_uso` (31/07), SQLSTATE `CG001` + as FKs `candidatos_cargo_id_fkey` e `cargo_apelidos_cargo_id_fkey`, ambas RESTRICT. ⚠️ **Reverte o ganho central do tema Cargos** — os 7 nomes com `¿` ficam permanentes depois da primeira importação. Ver [`../modulos/candidatos/cargos.md`](../modulos/candidatos/cargos.md) |
...
| Cargo com **qualquer** menção não se exclui — inscrito **ou** apelido | `candidatos.cargo_id` e `cargo_apelidos.cargo_id`, **as duas RESTRICT** (conferido no banco em 31/07). ⚠️ **Até 31/07 `cargo_apelidos` era CASCADE**, e a assimetria era deliberada ("apelido é atalho, candidato é gente"): excluir o cargo apagava a memória de "texto sujo → cargo" em silêncio. **Isso não vale mais** — menção é menção. Ver [`../modulos/candidatos/cargos.md`](../modulos/candidatos/cargos.md) |
