...
O projeto foi originalmente gerado pelo **Lovable** (plataforma low-code), mas em 2026-07-11 passou a ser mantido diretamente por nós: o `lovable-tagger` foi removido do `vite.config.ts`/`package.json`, o `.lovable/` e o README boilerplate saíram. Resquícios de scaffold ainda podem aparecer (nomes genéricos, comentários de "arquivo gerado") — trate como cruft, não como convenção a preservar. 🔵 **O sistema está NO AR desde 2026-08-13** (conferido na data: `https://fevre.online` responde 200 por nginx com TLS, e o bundle publicado aponta para o Supabase de produção). Servidor Ubuntu próprio, nginx servindo o build estático — o roteiro e o estado estão em [`hospedagem-e-deploy.md`](../../hospedagem-e-deploy.md). *(Esta frase dizia, até 13/08: "**Pendência:** não há deploy ativo em lugar nenhum… falta executar, e depende do bootstrap do banco novo". As duas dependências caíram — o banco em 10/08, a publicação em 13/08.)*
...
Desde 2026-07-24 a raiz `/` não abre mais uma lista, e sim um **hub** (`src/pages/Inicio.tsx`) que mostra a cada gestor os **módulos** a que ele tem acesso. 🔵 Desde 2026-09-23 existem **cinco** módulos: *Aplicação de Provas* (todas as rotas de gestão da tabela acima), *Editais* (`/editais`, só admin/superadmin), *Candidatos* (`/candidatos`, idem, criado em 2026-07-27), *Alocação de Candidatos* (`/alocacao-candidatos`, idem, criado em 2026-08-04) e *Financeiro* (`/financeiro`, criado em 2026-09-23 — **só superadmin + financeiro**, o único módulo a que `admin` comum NÃO tem acesso; ver [`../modulos/financeiro/00-modulo.md`](../modulos/financeiro/00-modulo.md)). A estrutura está pronta para os próximos: o valor está no **mecanismo**, não na lista.
...
- **Colaborador SEM papel de gestão nunca vê o hub** — cai direto em `/perfil-colaborador` (o guard do `Inicio.tsx` e o pós-login do `Auth.tsx` cuidam disso, pelo mesmo `isColaboradorSemGestao`). 🔵 Até 2026-09-18 a condição era `role === null`, que o trigger `handle_new_user` torna inalcançável — os 40 colaboradores comuns caíam no hub vazio. Ver a matriz papel × módulo em [`auth-e-permissoes.md`](./auth-e-permissoes.md).
...
- 🔴 **`public/auth_users_export.csv` TEM dado real** — ⚠️ esta linha afirmava, até 2026-08-08, que ele "contém só o cabeçalho (sem linhas de dados)" e que "não é vazamento de dados reais no momento". **Falso, e medido:** há uma linha com UUID de conta do Auth, e-mail, nome completo e data do último login. Como `public/` inteiro vai para o `dist/` e o `dist/` é o que o nginx serve, publicar o site o expõe em `https://fevre.online/auth_users_export.csv`. **Tirar de `public/` antes do primeiro deploy.** É o exemplo de afirmação **comportamental** que o `npm run docs:conferir` não pega: o arquivo existe, o caminho está certo, e a frase mente sobre o conteúdo.
...
- ~~**Deploy ainda no Lovable**~~ — 🔵 **RESOLVIDO em 2026-08-13.** A linha original dizia que "o site continua sendo publicado pela plataforma"; o site do Lovable deixou de existir em 12/07. 🔴 **Esta entrada depois afirmou, até 09/09, que *"desde então não há deploy ativo em lugar nenhum"* e que a hospedagem estava *"pendente de execução"* — as duas coisas falsas desde 13/08**, quando `fevre.online` entrou no ar em servidor próprio (nginx + TLS). O §1 deste mesmo arquivo já havia sido corrigido na época e esta linha não — **é o caso exemplar de corrigir num ponto e deixar o outro mentindo**. Estado atual em [`hospedagem-e-deploy.md`](../../hospedagem-e-deploy.md).
