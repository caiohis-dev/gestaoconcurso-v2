...
- **Ainda de pé, condenado:** a coluna `colab_codigo_acesso`, a tabela `colaborador_sessions` e a função `is_colaborador_logged_in` — os últimos restos, que saem no `DROP das sobras` + retirada da trava do UPDATE (o que falta da 2D). Não construa nada novo sobre elas.
