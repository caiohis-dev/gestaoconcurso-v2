...
- **Porta pública já trancada (subetapa 2D, item 1 — 2026-07-15):** as RPCs mortas do modelo `/auth` velho (`get_colaborador_full_data`, `update_colaborador_data_full`, `verify_colaborador_codigo_acesso`, `set_colaborador_password`, `update_colaborador_data` e todos os overloads, `verify_colaborador_password`, `check_colaborador_has_password`, `register/unregister/update_colaborador_session_activity`, etc.) tiveram o `EXECUTE` **revogado de `PUBLIC`** (migration `20260715072758_*`) — só o dono (`postgres`) as executa; `anon`/`authenticated` recebem `permission denied`. Isso fecha a **fragilidade 1** do laudo. Ficaram **de fora, ainda vivas:** `is_colaborador_logged_in` (usada em `useColaboradores.tsx` e na policy de UPDATE) e `get_coordenador_colaboradores` (lado gestão).
- **Ainda de pé, mas condenado:** as mesmas RPCs (agora trancadas), a coluna `colab_codigo_acesso`, a tabela `colaborador_sessions` e a função `is_colaborador_logged_in` **continuam existindo** — nada mais as usa desde a 2A. O `DROP` delas e a RLS de verdade em `colaboradores` são os próximos itens da subetapa D. Não construa nada novo sobre elas.
