...
**1. E-mails duplicados zerados (2026-07-14).** Nas **6 linhas** de `colaboradores` que compartilhavam **3 e-mails duplicados** (`suelenbertoldo9@gmail.com`, `yann_vr9@hotmail.com`, `teste@example.com` — dois colaboradores cada), o `colab_email` foi **zerado nos dois lados de cada par**. O motivo (um e-mail = um usuário no Supabase Auth; escolher um dos pares seria arbitrário) está em [`../analises/roadmap-auth-colaborador.md`](../analises/roadmap-auth-colaborador.md). Alterada **só** a coluna `colab_email` das 6 linhas; a `colab_chave_pix` foi preservada (a SOLANGE BERTOLDO RAIMUNDO usa o mesmo e-mail como chave PIX, dado bancário) e as 3 linhas de `email_atualizacao_log` também.
