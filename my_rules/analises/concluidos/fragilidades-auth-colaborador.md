...
É chamada com o `id` do `localStorage` na montagem, sem validar que a sessão é legítima — então o rastro de "está logado" (que bloqueia edição do admin, ver [`../../estrutura/modulos/aplicacao-provas/colaboradores.md`](../../estrutura/modulos/aplicacao-provas/colaboradores.md)) pode ser disparado para qualquer `id`.
