...
>
> 🔴 **E uma regra do mesmo dia inverte o sentido de todo o bloco abaixo: cargo com QUALQUER menção em outra tabela virou IMUTÁVEL** — migration `20260731100000`, trigger `check_cargo_nao_alteravel_em_uso` (SQLSTATE `CG001`), e `cargo_apelidos_cargo_id_fkey` passou de CASCADE para RESTRICT. **Toda afirmação abaixo de que renomear cargo é "inofensivo" ou "deixou de duplicar candidato" descreve o que a etapa 5 entregou, não o que vale hoje**: renomear cargo com inscritos ou apelidos é simplesmente recusado. Como **os 9 cargos do arquivo real têm candidatos**, depois da primeira importação os **7 nomes com `¿` ficam permanentes** — o ganho central deste tema. Foi decidido pelo usuário com esses números à vista; a janela para corrigir um nome passou a ser **antes** da primeira importação que o use, na prática no passo 3 do assistente.
