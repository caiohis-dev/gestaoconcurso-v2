...
- **`DROP` das sobras:** **[✅ parcial em 2026-07-15]** dropadas `is_colaborador_logged_in` e a tabela `colaborador_sessions` (mesma migration `20260715130603_*`); template órfão `_shared/transactional-email-templates/codigo-acesso.tsx` removido. **Falta a coluna `colab_codigo_acesso` (+ CHECK `colab_codigo_acesso_format`)** — adiada de propósito: é referenciada por dois exports e pelo e-mail em massa, então cai junto com o item 6 abaixo.
- **Repensar o e-mail em massa do `PainelDadosColaboradores.tsx` + dropar a coluna:** o e-mail ainda embute `colab_codigo_acesso` (morto) e aponta para `fevre.online/auth` no modelo antigo — remover o código do corpo e apontar para primeiro-acesso/reivindicação, ou aposentar o botão. **Decidido isso**, tirar a coluna `colab_codigo_acesso` das duas planilhas de export (`GerenciarProva`, `GerenciarColaboradoresProva`) e do tipo/erros do `useColaboradores`, e então **dropar a coluna** (fecha o resto do item 5).
