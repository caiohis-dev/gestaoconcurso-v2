...
Hoje, vivos: [`dividas-auth-colaborador.md`](./dividas-auth-colaborador.md), [`roadmap-edicao-email-colaborador.md`](./roadmap-edicao-email-colaborador.md), [`roadmap-alocacao-por-arrasto.yaml`](./roadmap-alocacao-por-arrasto.yaml) (🟢 executado em 2026-08-05, aguardando merge para ir a `concluidos/`), [`roadmap-lista-de-candidatos-e-exclusao.yaml`](./roadmap-lista-de-candidatos-e-exclusao.yaml) (⏳ **não iniciado** — desenho aprovado, 3 decisões de modelo ainda a confirmar), [`roadmap-alternar-apontamento-do-banco.yaml`](./roadmap-alternar-apontamento-do-banco.yaml) (⏳ **não iniciado** — script gitignored para apontar o `dev` ao banco local ou à nuvem; 🔴 leia o R1 antes: com ele ligado, todo clique escreve num banco **sem backup**) e [`roadmap-n-inscricao-12-caracteres.yaml`](./roadmap-n-inscricao-12-caracteres.yaml) (🟢 **executado em 2026-08-06** — `n_inscricao` virou `text` com teto de 12; aguardando merge para ir a `concluidos/`).

➕ [`roadmap-rate-limit-fluxos-de-acesso.yaml`](./roadmap-rate-limit-fluxos-de-acesso.yaml) (⏳ **não iniciado**, 2026-08-13) — o plano de execução em 5 etapas, com o que já foi medido, as 3 decisões de número que faltam e as 7 alternativas recusadas com motivo. **É por ele que se começa**; o arquivo abaixo é o porquê.
...
➕ [`analise-ultimo-acesso-e-convite.md`](./analise-ultimo-acesso-e-convite.md) (🟢 **o carimbo foi EXECUTADO**, ⏳ a trilha de envio não, 2026-09-20) — a coluna `colab_ultimo_acesso` não tem escritor desde 15/07 e a tela `/colaboradores` erra nos dois sentidos. Guarda a medição dos carimbos do GoTrue (o clique no convite **limpa** `confirmation_sent_at` — é o melhor sinal de "convite não aberto") e **um limiar meu que o controle positivo reprovou**: propus 2 minutos para separar "só clicou" de "usa o sistema", e o dado mostrou que o corte é de **1 segundo**. 🔴 Guarda também o achado da implementação: os dois blocos `EXCEPTION` do gatilho de login têm de ser **separados**, senão a exceção do carimbo **desfaz o vínculo junto**.
