...
🔵 **O limite de e-mail do Free não atinge este sistema, e isso não é óbvio.** O SMTP embutido do Supabase é 2 mensagens/hora e **só entrega para endereços de membros do time** — o que seria fatal para um sistema que convida fiscais por e-mail. Mas nenhum fluxo daqui depende dele: `_shared/enviar-link-acesso.ts` usa `auth.admin.generateLink`, que **gera o link sem enviar nada**, e o envio vai pela Edge Function `send-email` → SMTP da Hostinger. `create-admin` e `create-coordenador` criavam a conta com `email_confirm: true` (nenhum e-mail) — 🔵 as duas saíram do repo (12/09 e 24/09); a sucessora da primeira, `conceder-papel-sistema`, cria a conta pelo mesmo `generateLink` do helper, então segue fora do SMTP nativo. O único caminho nativo, `supabase.auth.signUp` em `src/hooks/useAuth.tsx`, **nunca é chamado**. Ainda assim, cadastre o SMTP da Hostinger como *Custom SMTP* no dashboard (passo 6): é grátis, e cobre convite disparado pelo próprio dashboard.
...
>
> ⚠️ **Achado do `prod:diff` deste dia, e ele NÃO veio deste push:** o diff propõe `drop extension if exists "pg_net"`. Produção **tem** a extensão `pg_net` e **nenhuma migration a declara** — drift pré-existente, quase certamente do keep-alive de 09/09, que foi montado pelo dashboard. 🔴 **Não execute esse drop.** Ele é sugestão do `migra` para fazer o remoto igual ao repo, e aqui quem está incompleto é o repo. Regularizar isso (uma migration `CREATE EXTENSION IF NOT EXISTS pg_net`) é item para quem for mexer no keep-alive — sem ela, todo `prod:diff` futuro vai repetir esse aviso e treinar quem lê a ignorá-lo.
