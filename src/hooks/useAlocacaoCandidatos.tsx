...
/**
 * A distribuição automática. Roda INTEIRA no banco (o payload é um uuid) e devolve as
 * três contagens. As recusas (AL001..AL004, PF001) já chegam em português dizendo o que
 * fazer — o toast as repassa intactas.
 */
export function useDistribuirCandidatos() {
  const queryClient = useQueryClient();
  const { toast } = useToast();

  const mutation = useMutation({
    mutationFn: async (provaId: string) => {
      const { data, error } = await supabase.rpc("distribuir_candidatos_da_prova", {
        p_prova_id: provaId,
      });
      if (error) throw error;
      // RETURNS TABLE com uma linha: o PostgREST entrega como array.
      const linha = Array.isArray(data) ? data[0] : data;
      return linha as { alocados: number; preservados: number; pendentes_especiais: number };
    },
    onSuccess: (r) => {
  ...
  return { distribuir: mutation.mutateAsync, isDistribuindo: mutation.isPending };
}
