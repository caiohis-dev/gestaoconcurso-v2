...
**Notas:** ⚠️ **Nenhum caso desta bateria cai mais no Mailpit** — esta linha dizia que `C2` e `D2` dependiam dele, o que ficou falso e contradizia o próprio texto do `C2`. Os três caminhos de e-mail (`C2` reivindicação, `D2` cadastro público, `E1/E2` recuperação) passam pelo helper `_shared/enviar-link-acesso.ts` → `send-email` → **Hostinger**, e portanto **saem de verdade**. O Mailpit (`http://127.0.0.1:54324`) só recolhe o que o GoTrue ainda compõe sozinho, e nada nesta bateria está nessa categoria. Para evitar **qualquer** envio real, teste só até a tela de "confira o e-mail" e valide o vínculo direto no banco. **H3**, **I7**, **J7** e **K1** precisam de terminal (curl/psql), não de UI. Os blocos **E** e **J** exigem `npm run supabase:functions:serve` além do `npm run dev`.
